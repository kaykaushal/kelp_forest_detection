...
from src.train import Trainer, create_writer
...
df = meta_df[meta_df['in_train'] == True].sample(1000)
...
def objective(trial):
    ...
    out_channels = 1
    features = [32,64,128,256]#[trial.suggest_int(f'features_{i}', 32, 512) for i in range(4)]
    dropout_prob = 0.0 #trial.suggest_float('dropout_prob', 0.0, 0.5)
    # Define model
    model = UNetPlus(
    ...
    lr = 8.5e-4 #trial.suggest_float('lr', 1e-4, 1e-3, log=True)
    # Choose optimizer and its hyperparameters
    optimizer_name = trial.suggest_categorical('optimizer', ['Adamax', 'Adam']) #, 'Adam', 
    ...
    loss_name = trial.suggest_categorical('loss', ['BCELoss', 'MSELoss']) #
    ...
    trainer = Trainer(
        ...
        optimizer=optimizer,
        device=device,
        writer = create_writer(experiment_name="Full_Channel", model_name="UnetPlus", extra=f"{loss_name}_{optimizer_name}"),
    ...
    trainer.train(epochs=20)  # Adjust the number of epochs as needed

    # Return a scalar value indicating the performance
    return trainer.best_valid_dice

if __name__ == "__main__":
    print(len(train_dataloader))
    # Create an optimization study and perform optimization
    torch.cuda.empty_cache()
    study = optuna.create_study(direction='maximize')  # Optimize for minimum validation loss
